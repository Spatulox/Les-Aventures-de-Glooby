...
**Point crucial : les musiques ne sont PAS dans les nœuds de la scène.** Un nœud `CameraZone` de `01-monde1.tscn` ne porte qu'une **clé texte** (`NomAmbiance`, ex. `"village"`) ; les pistes elles-mêmes vivent dans des **ressources `.tres`** de `assets/audio/ambiances/` (une par lieu). Pour changer ce qui joue, on édite le `.tres` (dock FileSystem), pas la scène.
...
- **`niveaux/`** — playable levels, **préfixés par leur ordre de traversée** (`EcranScenesDebug` trie par nom, donc le préfixe = l'ordre affiché) : `01-monde1.tscn`, l'arène `02-BossReindeer.tscn`, `03-monde2.tscn`, l'arène finale `04-BossEnd.tscn`. `EcranScenesDebug` scans this folder, so a new level shows up in the debug menu with no code change.
