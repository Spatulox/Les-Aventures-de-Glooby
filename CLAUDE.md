...
**`LivingEntity.cs`** (`scripts/Entities/`, abstract `CharacterBody2D`, implements `Damageable`) — the shared base for everything that "lives": both `Player` and `Boss` (and any future PNJ) extend it. It owns `PvMax`/`Pv`/`EstVaincu`, the `PvChanges`/`Vaincu` signals, `DefinirPvMax`, the damage pipeline (`TakeDamage`/`SubirDegats(int)` with `AjusterDegats`/`ApresDegats`/`Mourir` virtual hooks), and reusable movement helpers `AppliquerGravite`/`AppliquerFriction`/`Sauter` (tunables `Gravity`/`MaxFallSpeed`/`Friction`/`JumpVelocity`). The generic `Mourir()` marks the entity beaten, zeroes velocity and emits `Vaincu`.

- **`Player.cs`** (`scripts/Entities/Player/`, `: LivingEntity`) — timer-based controller: coyote time + jump buffer, accelerated slide (faster on `is_ice`), snowball throw, short post-hit invincibility, fragile-tile break delay, relative fall-death safety net. Uses the base movement helpers. Its HP lives in **`GameState`, not the base `Pv`** (persistent, HUD-bound, respawn), so it overrides `SubirDegats(int)` to route damage there and `IsInvincibleToDamage` to its invincibility timer; `Blesser(int direction, int quantite)` is the knockback-carrying entry point called by the boss/hazards. Tunables are `[Export]` fields at the top.
